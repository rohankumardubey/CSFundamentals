...
/**
 * Given a string, the task is to find maximum consecutive repeating character
 * in string.
 * 
 * Note : We do not need to consider overall count, but the count of repeating
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (i < s.length() - 1 && s.charAt(i) == s.charAt(i + 1)) {
				...
				currentCount++;
			} else { // If the current character does not match with the next, update the result and
						// its count
